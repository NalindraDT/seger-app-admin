...
export default function Modal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/45 p-4 backdrop-blur-sm animate-in fade-in duration-200">
      <div className={`flex max-h-[92vh] w-full flex-col overflow-hidden rounded-2xl border border-gray-100 bg-white shadow-2xl ${sizes[size]} ${className}`}>
        <div className="flex items-start justify-between border-b border-gray-100 bg-gradient-to-r from-[#F8F9FC] to-white px-6 py-4">
        ...
        {footer && (
          <div className="flex gap-3 border-t border-gray-100 bg-gray-50/80 px-6 py-4">{footer}</div>
        )}
      </div>
    </div>
  );
